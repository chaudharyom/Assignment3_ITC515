...
public class Loan implements ILoan {
    ...
    private boolean sane(IBook book, IMember borrower, Date borrowDate, Date returnDate) {
        return (book != null)
                && (borrower != null)
                && (borrowDate != null)
                && (returnDate != null) && (borrowDate.compareTo(returnDate) <= 0);
    }
    ...
    public String toString() {
        return String.format("Loan ID:  %d\nAuthor:   %s\nTitle:    %s\nBorrower: %s %s\nBorrowed: %s\nDue Date: %s", new Object[]{
            Integer.valueOf(this.id), this.book.getAuthor(), this.book.getTitle(), this.borrower.getFirstName(), this.borrower.getLastName(),
            DateFormat.getDateInstance().format(this.borrowDate),
            DateFormat.getDateInstance().format(this.dueDate)});
    }
}
